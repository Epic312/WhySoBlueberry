package com.evoslab.why_so_blueberry.common.block;
...
public class BlueberryBushBlock extends BushBlock implements IPlantable, IGrowable {
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        ...
        if (!fullyGrown && player.getHeldItem(handIn).getItem() == Items.BONE_MEAL) {
            ...
            Item blueberry = state.get(TYPE) == BlueberryType.PINK ? WhySoBlueberryItems.PINK_BLUEBERRIES.get() : WhySoBlueberryItems.BLUEBERRIES.get();
            ...
            if (age < maxAgeForPos) {
                if (net.minecraftforge.common.ForgeHooks.onCropsGrowPre(worldIn, pos, state, rand.nextInt(growthChance) == 0)) {
                    if (age != 3) {
                        ...
                        worldIn.setBlockState(pos, this.withAge(age + 1).with(TYPE, this.isPink(worldIn, pos) ? BlueberryType.PINK : BlueberryType.BLUE), 2);
                    }
    ...
    protected IItemProvider getSeedsItem() {
        return WhySoBlueberryItems.BLUEBERRY_PIPS.get();
    }
    ...
    private boolean isPink(ServerWorld worldIn, BlockPos pos) {
        return (pos.getY() >= 200 && worldIn.func_234922_V_() == DimensionType.field_235999_c_) || worldIn.func_234922_V_() == DimensionType.field_236001_e_;
    }
    ...
    public void grow(ServerWorld worldIn, Random rand, BlockPos pos, BlockState state) {
        ...
        if (age != 5 && age != 4) {
            ...
            worldIn.setBlockState(pos, this.withAge(age).with(TYPE, this.isPink(worldIn, pos) ? BlueberryType.PINK : BlueberryType.BLUE), 2);
        }
    }
